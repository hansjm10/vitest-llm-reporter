...
describe('ConsoleCapture Bug Fixes', () => {
  ...
  describe('Race condition prevention', () => {
    it('should not clear buffer if test ID is reused', async () => {
      ...
      expect(output1.entries).toBeInstanceOf(Array)
      expect(output1.entries.some((e) => e.message === 'First test')).toBe(true)
      ...
      expect(output2.entries).toBeInstanceOf(Array)
      expect(output2.entries.some((e) => e.message === 'Second test')).toBe(true)
      // Should NOT contain output from first test
      expect(output2.entries.some((e) => e.message === 'First test')).toBe(false)
