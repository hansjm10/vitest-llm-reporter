...
export class WarningSystem {
  ...
  warnModelLimit(
    ...
    const severity = limitType === 'context' ? 'high' : 
                     limitType === 'recommended' ? 'medium' : 'low';
...
export class WarningFormatter {
  ...
  static formatConsole(warning: MetricsWarning): string {
    const emoji = warning.severity === 'high' ? '🚨' : 
  ...
  static formatSummary(stats: WarningStats): string {
    ...
    const parts = [
      `⚠️ ${stats.total} warning${stats.total !== 1 ? 's' : ''} detected`,
    ];
 ...
 */
export function getWarningSystem(options?: { maxWarnings?: number; maxErrors?: number }): WarningSystem {
  if (!defaultSystem) {
