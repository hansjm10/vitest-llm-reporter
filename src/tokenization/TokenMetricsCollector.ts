...
import { ThresholdManager, getThresholdManager, createModelAwareThresholds } from './metrics/thresholds.js';
...
export class TokenMetricsCollector {
  ...
  constructor(config: TokenMetricsConfig, events: CollectionEvents = {}) {
    ...
    this.thresholdManager = getThresholdManager(
      createModelAwareThresholds(this.config.model)
    );
  ...
  async processTest(
    ...
    try {
      ...
      const totalTokens = Object.values(sectionMetrics).reduce((sum, section) => sum + section.count, 0);
  ...
  async finalize(): Promise<TokenMetrics> {
    ...
    try {
      ...
      if (this.batchAggregator && this.testMetrics.size > 100) {
        ...
        const results = await this.batchAggregator.aggregateInBatches(testsByFile, aggregationOptions);
      ...
      logger(`Collection complete. Total tokens: ${summary.totalTokens}, Files: ${fileMetrics.length}, Tests: ${summary.testCounts.total}`);
  ...
  getCurrentMetrics(): Partial<TokenMetrics> {
    ...
    const files = this.streamingAggregator?.getCurrentFiles() || Array.from(this.fileMetrics.values());
  ...
  exportMetrics(
    ...
    switch (options.format) {
      ...
      case 'jsonl':
        ...
        const lines: string[] = [];
        if (exportData.summary) lines.push(JSON.stringify({ type: 'summary', data: exportData.summary }));
        if (exportData.files) {
  ...
  getCollectionStats(): MetricsStats {
    const baseStats = this.aggregator?.getStats() || this.batchAggregator?.getStats() || {
    ...
    for (const [section, content] of Object.entries(sections) as [MetricSection, string][]) {
      ...
      try {
        ...
        if (content.length > truncatedContent.length) {
          this.warningSystem.warnContentTruncated(
            content.length,
  ...
  private checkTestThresholds(
    ...
    for (const [section, sectionData] of Object.entries(sections) as [MetricSection, SectionTokens][]) {
