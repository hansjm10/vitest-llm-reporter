...
describe('Deduplication Integration', () => {
  ...
  describe('Configuration Strategy Testing', () => {
    it('should behave differently with conservative strategy', async () => {
      ...
      expect(conservativeResult.deduplicatedCount).toBeGreaterThanOrEqual(Math.floor(conservativeResult.originalCount * 0.5))
    })
  ...
  describe('Concurrent Processing', () => {
    it('should handle concurrent deduplication requests', async () => {
      const outputs = Array.from({ length: 5 }, (_, i) => 
        createSampleOutput(0, 10 + i * 5, 0)
      )
      
      const promises = outputs.map(output => 
        services.deduplicationService.processOutput(output)
      )
